...
bucket_name = 'topictimelinebucket' 

def load_from_s3(bucket_name, key):
    """从S3下载数据并使用pickle反序列化"""
    # 初始化S3客户端
    s3_client = boto3.client('s3')
    # 创建一个BytesIO对象用于存储从S3读取的二进制数据
    buffer = io.BytesIO()
    # 下载数据到buffer
...
def retrival():
    ...
    # 从s3加载矩阵向量
    save_directory = f'{start}_{end}'

    glossary = load_from_s3(bucket_name, f'{save_directory}/{start}_{end}_glossary.pkl')
    weigh_list = load_from_s3(bucket_name, f'{save_directory}/{start}_{end}_weigh_list.pkl')
    weigh_matrix = load_from_s3(bucket_name, f'{save_directory}/{start}_{end}_weigh_matrix.pkl')
    inverse_index = load_from_s3(bucket_name, f'{save_directory}/{start}_{end}_inverse_index.pkl')
    jieba_seg = load_from_s3(bucket_name, f'{save_directory}/{start}_{end}_jieba_seg.pkl')
